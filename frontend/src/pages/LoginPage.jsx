...
export default function LoginPage() {
    ...
    return (
        <div style={{ minHeight: "100vh", background: T.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Space Grotesk',sans-serif", padding: 20 }}>
            <style>{CSS}</style>
            <Helmet><title>Login — Stock Fortress</title></Helmet>
            <div style={{ width: "100%", maxWidth: 400, animation: "fi .5s ease both" }}>
                <div style={{ textAlign: "center", marginBottom: 30 }}>
                    ...
                    <h1 style={{ fontSize: 24, fontWeight: 700, color: T.text, marginBottom: 8 }}>Welcome back</h1>
                    <p style={{ fontSize: 14, color: T.textSec }}>Log in to access your saved reports</p>
                </div>

                <form onSubmit={handleLogin} style={{ background: T.card, padding: 30, borderRadius: 16, border: `1px solid ${T.border}`, boxShadow: "0 4px 20px rgba(0,0,0,0.02)" }}>
                ...
                <div style={{ textAlign: "center", marginTop: 24, fontSize: 14, color: T.textSec }}>
                    Don't have an account? <Link to="/signup" style={{ color: T.accent, fontWeight: 600, textDecoration: "none" }}>Sign up</Link>
                </div>
            </div>
        </div>
    );
